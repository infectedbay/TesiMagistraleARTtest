import java.io.File;
import java.io.IOException;
import java.util.*;

public class Main {

    // Input files
    static final File file_attributes = new File("./src/data/attributes.txt");
    static final File file_plans = new File("./src/data/plans.txt");
    static final File file_data = new File("./src/data/data.txt");
    static final File folder_rules = new File("./src/data/rules");
    static final File folder_relations_of_preference = new File("./src/data/relations_of_preference");
    static final File folder_list_of_weight_function = new File("./src/data/weight_function");
    static final File file_hierarchy = new File("./src/data/hierarchy.txt");
    static final File file_global_rules = new File("./src/data/global_rules.txt");
    static final File file_plan_cost = new File("./src/data/price.txt");
    static final File file_data_space = new File("./src/data/space.txt");


    // Python file for CP-SAT
    static final File file_python = new File("./python/result.py");

    // Colors for System.out.print("")
    public static final String ANSI_RESET = "\u001B[0m";
    public static final String ANSI_RED = "\u001B[31m";
    public static final String ANSI_GREEN = "\u001B[32m";
    public static final String ANSI_YELLOW = "\u001B[33m";
    public static final String ANSI_BLUE = "\u001B[34m";
    public static final String ANSI_PURPLE = "\u001B[35m";
    public static final String ANSI_CYAN = "\u001B[36m";
    public static final String ANSI_WHITE = "\u001B[37m";

    // Maximum number of cycles for finding a solution with global rule TOGETHER, NOT_TOGETHER and TOGETHER*
    // It also limit maximum number of cycles for finding better solution compliant to hierarchy (ALSO FOR ONLY_ONE)
    static final int MAX_CYCLE = 150000;
    // If true, look for a solution (for TOGETHER, NOT_TOGETHER and TOGETHER*) ignoring the maximum number of cycles.
    // ATTENTION MAY CAUSE AN INFINITE LOOP IF THERE IS NO SOLUTION
    //      (FOR EXAMPLE IF THERE IS A CYCLE WITH TOGETHER AND NOT TOGETHER RULES)
    static final boolean INFINITE_LOOP = true;
    // (For rules ONLY_ONE rules we cycle only if exist a solution, INFINITE_LOOP and MAX_CYCLE is ignored)
    // Search results in java code (is un-efficient)
    static final boolean RESULT = false;
    // To evaluate performance
    static final boolean TIME_TEST = false;
    static final String DOMINANCE = "D";

    // Rule list
    static final ArrayList<String> rules_list = new ArrayList<>() {{
        add("ALL");
        add("ANY");
        add("IF-THEN");
        add("FORBIDDEN");
        add("AT_MOST");
        add("AT_LEAST");
    }};

    // Global rule list
    static final ArrayList<String> global_rules_list = new ArrayList<>() {{
        add("TOGETHER");
        add("TOGETHER*");
        add("ALL_TOGETHER*");
        add("NOT_TOGETHER");
        add("NOT_TOGETHER*");
        add("SPLIT");
        add("ALL_SPLIT");
        add("ALONE");
    }};

    public static void main(String[] args) throws IOException {
    // Attribute -> Values
        LinkedHashMap<String,ArrayList<String>> attributes = new LinkedHashMap<>();
        ArrayList<String> attributeOrderList = new ArrayList<>();
    // Plan name -> Values
    // Plan Definition according Definition 3.1.1 at pag.28
        LinkedHashMap<String,ArrayList<String>> plans = new LinkedHashMap<>();
    // List of data type
        ArrayList<String> dataOrderList = new ArrayList<>();
    // Data Type -> List of SetOfRules on the attribute
        LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements = new LinkedHashMap<>();
    // Acceptable plans : SetOfRules -> List of acceptable plans
        LinkedHashMap<SetOfRules,ArrayList<String>> setOfAcceptablePlans = new LinkedHashMap<>();
    // Restricted attribute domain for each rule set
    // SetOfRules -> Attribute -> Value list
        LinkedHashMap<SetOfRules,LinkedHashMap<String,ArrayList<String>>> restrictedDomain = new LinkedHashMap<>();
    // Preference relation
    // SetOfRules -> Attribute -> List of Preference
        LinkedHashMap<SetOfRules,LinkedHashMap<String,Preference>> preferences = new LinkedHashMap<>();
    // For each set of requirements, score vector of the plans
    // SetOfRules -> Plan -> ScoreVector (p_top is ideal plans)
        LinkedHashMap<SetOfRules,LinkedHashMap<String,ScoreVector>> scoreVectors = new LinkedHashMap<>();
    // Ranking of plans based on score vector
    // SetOfRules -> Ordered ArrayList of plans
        LinkedHashMap<SetOfRules,ArrayList<String>> ranking = new LinkedHashMap<>();
    // Allocation function
    // SetOfRules -> List plans allocated for the respective set of requirements
        LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction = new LinkedHashMap<>();
    // Hierarchy
    // Data type -> hierarchy of SetOfRules
        LinkedHashMap<String,ArrayList<SetOfRules>> hierarchy = new LinkedHashMap<>();
    // Cost of al plan for a GB
        LinkedHashMap<String,Integer> planCost = new LinkedHashMap<>();
    // Storage (in GB) required by each resource
        LinkedHashMap<String,Integer> dataSpace = new LinkedHashMap<>();
    // List of global rules
        ArrayList<GlobalRule> globalRules = new ArrayList<>();

    // Legend
        System.out.println();
        System.out.println("Legend : ");
        System.out.println(ANSI_PURPLE + "\tPurple is to highlight parts of text" + ANSI_RESET);
        System.out.println(ANSI_CYAN + "\tCyan is for processing results" + ANSI_RESET);
        System.out.println(ANSI_BLUE + "\tBlue is also for processing results" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "\tGreen is for passed tests" + ANSI_RESET);
        System.out.println(ANSI_YELLOW + "\tYellow is for breaks or warnings" + ANSI_RESET);
        System.out.println(ANSI_RED + "\tRed is for critical errors" + ANSI_RESET);
        System.out.println();

    // Read data from attributes.txt
        ReadPrintParse.read_attributes(attributes, attributeOrderList, file_attributes);
        ReadPrintParse.print_attributes(attributes, "Set of attributes (A) includes :" );
        System.out.println();

    // Read data from plans.txt
        ReadPrintParse.read_plans(plans, file_plans);
        ReadPrintParse.print_plans(plans, "Set of Plans (P) includes :");
        System.out.println();

    // Read plan cost
        ReadPrintParse.read_plan_cost(file_plan_cost, planCost);
        ReadPrintParse.print_plan_cost_data_space("Cost in euro of one gigabyte for each plan: ", planCost);
        System.out.println();

    // Read data from da data.txt
        ReadPrintParse.read_data(dataOrderList, file_data);
        ReadPrintParse.print_data(dataOrderList, "Set of Data (D) includes :");
        System.out.println();

    // Read space required by data
        ReadPrintParse.read_data_space(file_data_space, dataSpace);
        ReadPrintParse.print_plan_cost_data_space("Storage space required by data", dataSpace);
        System.out.println();

    // Check if rules syntax is correct
        ReadPrintParse.rules_parser(Objects.requireNonNull(folder_rules.listFiles()),
                dataOrderList,
                attributeOrderList,
                attributes);
        System.out.println(ANSI_GREEN + "Syntax of rules files is correct."  + ANSI_RESET);

    // Read data from the folder ./rules
        ReadPrintParse.read_rules(Objects.requireNonNull(folder_rules.listFiles()), setOfRequirements, attributeOrderList);
        System.out.println();
        ReadPrintParse.print_set_of_rules(setOfRequirements,"Set of Requirements (R) includes :");

    // Check if set of requirements is well-defined as definition 3.2.1 pag.33
    // In each requirement's set there is at most 1 occurrence of an attribute for all terms and rules "ALL".
    // Each rule must contain at most 1 occurrence of an attribute.
        System.out.println();
        check_if_requirements_are_well_defined(setOfRequirements);

    // Search for acceptable plans
    // Identifies acceptable plans according to definition 3.3.3 at pag.40
        acceptable_plans(setOfRequirements, plans, attributeOrderList, setOfAcceptablePlans);
        System.out.println();
        ReadPrintParse.print_acceptable_plans(setOfAcceptablePlans);
        check_if_there_enough_acceptable_plans(setOfAcceptablePlans,setOfRequirements);
        System.out.println();
        System.out.println(ANSI_GREEN + "Set of acceptable plans (P) is defined according to definition 3.3.3" + ANSI_RESET);
        System.out.println(ANSI_GREEN + "The number of acceptable plans for each set of requirements is sufficient." + ANSI_RESET);

    // Identifies restricted domains according to definition 3.4.1 at pag.42
        //restricted_domain(restrictedDomain,attributes,setOfRequirements);
        //System.out.println();
        //ReadPrintParse.print_restricted_domain(restrictedDomain, attributeOrderList);
        //System.out.println();
        //System.out.println(ANSI_GREEN + "Restricted domain is defined according to definition 3.4.1" + ANSI_RESET);

    // Relation of preferences
    // If the preference relation is not defined for an attribute in a set of requirements, the default order is used.
    // Define set of preference relations defined in definition 3.4.2 pag 43
    // Calculate score function according definition 3.4.3 pag.46
    // Define weight function according definition 3.4.4. pag.47
        //preference_relation(preferences,restrictedDomain,attributes,
        //        Objects.requireNonNull(folder_relations_of_preference.listFiles()),
        //        Objects.requireNonNull(folder_list_of_weight_function.listFiles()));
        //System.out.println();
        //ReadPrintParse.print_preferences(preferences, attributeOrderList);
    // If everything is ok, the following definitions are checked
        //System.out.println();
        //System.out.println(ANSI_GREEN + "Set of preference relations is defined according to definition 3.4.2" + ANSI_RESET);
        //System.out.println(ANSI_GREEN + "Score function is defined according to definition 3.4.3" + ANSI_RESET);
        //System.out.println(ANSI_GREEN + "Weight function is defined according to definition 3.4.4" + ANSI_RESET);

    // Choose whether you want to use DP-Dominance or D-Dominance
        //String dominantRelationship = "";
        //long start = 0;
        //long end = 0;
        //if(!TIME_TEST) {
        //    while (!dominantRelationship.equals("D") && !dominantRelationship.equals("DP")) {
        //        System.out.println();
        //        dominantRelationship = pause("Choose the dominant relationship you prefer " +
        //                "\n\t Type \"DP\" for DP-Dominance or \"D\" for D-Dominance : ");
        //    }
        //} else {
        //    start = System.currentTimeMillis();
        //    dominantRelationship = DOMINANCE;
        //}

    // The score vector is calculated for each acceptable plane for each set of requirements
        //score_vector_calculation(dominantRelationship,attributeOrderList,plans,setOfAcceptablePlans,preferences,scoreVectors);
        //System.out.println();
        //ReadPrintParse.print_score_vector(scoreVectors,attributes,dominantRelationship);
        //System.out.println(ANSI_GREEN + "Ideal plans are defined according to definition 3.4.5" + ANSI_RESET);
        //System.out.println(ANSI_GREEN + (dominantRelationship.equals("D") ? "S" : "Weighted s") + "core vectors are defined according to definition " + (dominantRelationship.equals("D") ? "3.4.6" : "3.4.8") + ANSI_RESET);


    // For each set of requirements, the classification based on the dominance relationship D-Dominance
    // (or the dominance relationship DP-Dominance) is calculated
        //calculate_ranking(scoreVectors,ranking);
        //System.out.println();
        //ReadPrintParse.print_ranking(scoreVectors,ranking,dominantRelationship);
        //System.out.println(ANSI_GREEN + "Ranking is calculated with " + (dominantRelationship.equals("D") ? "D-Dominance" : "DP-Dominance") +
        //        " relationship defined according to definition " +
        //        (dominantRelationship.equals("D") ? "3.4.7" : "3.4.9") + ANSI_RESET);

    // Java Solver
        //if(RESULT || TIME_TEST) {
        // Definition of allocation function without considering global requirements
            //allocation_function_no_global_rules(setOfRequirements, ranking, allocationFunction);
            //System.out.println();
            //ReadPrintParse.print_allocation_function("Allocation function (L) without considering global requirements : ",
            //        allocationFunction, dominantRelationship);
            //System.out.println();
        // Check if solution found is correct for requirements
            //if (check_no_global_rules(allocationFunction))
            //    System.out.println(ANSI_GREEN + "Allocation Function is defined according to definition 3.4.10" + ANSI_RESET);
            //else
            //    stop("There is a set of requirements of type \"o\" that have allocated less than 1 plan or \n" +
            //            "There is a set of requirements of type \"c\" that have allocated less than n_p plans! \n");
        //}

        // Read hierarchy from hierarchy.txt and print it (if is defined)
            //ReadPrintParse.read_hierarchy(file_hierarchy, hierarchy, setOfRequirements);
            //if (!hierarchy.isEmpty()) {
            //    System.out.println();
            //    ReadPrintParse.print_hierarchy(hierarchy);
            //}

        // Read global rules from global_rules.txt
            ReadPrintParse.read_global_rules(file_global_rules, globalRules, setOfRequirements);
            System.out.println();
            ReadPrintParse.print_global_rules(globalRules);

    // Java Solver Part 2
        //if (RESULT || TIME_TEST) {
        // Definition of allocation function considering all global requirements of type ONLY_ONE
        // Check if exist a solution with ONLY_ONE rules
            //check_if_exist_a_solution_with_only_one_rules_with_print(setOfRequirements, ranking, globalRules);
            //// Check if exist a solution with ONLY_ONE rules without printing
            ////check_if_exist_a_solution_with_only_one_rules(setOfRequirements,ranking,globalRules);
            //System.out.println();
            //System.out.println(ANSI_GREEN + "Exist a solution considering ONLY_ONE RULES." + ANSI_RESET);
            //allocation_function_only_one(ranking, hierarchy, globalRules, allocationFunction);
            //System.out.println();
            //ReadPrintParse.print_allocation_function("Allocation function (L) considering only global requirements of type ONLY_ONE : ",
            //        allocationFunction, dominantRelationship);
            //System.out.println();
        // Check if solution found is correct for ONLY_ONE rules and for requirements
            //if (check_only_one(globalRules, allocationFunction) && check_no_global_rules(allocationFunction)) {
            //    System.out.println(ANSI_GREEN + "Allocation function respects ONLY_ONE rules." + ANSI_RESET);
            //} else {
            //    stop("Allocation function does not respect ONLY_ONE rules!");
            //}

        // Definition of allocation function considering all global requirements
            //allocation_function_global_rules(ranking, hierarchy, globalRules, allocationFunction);
            //System.out.println();
            //ReadPrintParse.print_allocation_function("Allocation function (L) considering global requirements : ", allocationFunction, dominantRelationship);
            //System.out.println();
        // Check if solution found is correct for all global rules and for requirements
            //if (check_all_rules(globalRules, allocationFunction))
            //    System.out.println(ANSI_GREEN + "Allocation function respects all global rules. \n" +
            //            "Allocation is correct as defined in definition 3.5.2" + ANSI_RESET);
            //else
            //    stop("Allocation function does not respect global rules!");
        //}

    // Write python code for OR-Tools SAT-Solver
        if (!TIME_TEST) {
            System.out.println();
            PythonART.writeFile(file_python, setOfRequirements, setOfAcceptablePlans, globalRules, planCost, dataSpace);
            System.out.println("To find the result using the SAT-Solver \n\t1. Install Google’s OR-Tools at the link : https://developers.google.com/optimization/install");
            System.out.println("\t2. Open CMD and paste the following commands");
            System.out.println("\t\tcd " + (file_python.getAbsolutePath().substring(0, file_python.getAbsolutePath().length() - file_python.getName().length())));
            System.out.println("\t\tpython result.py");
        } else {
            //end = System.currentTimeMillis();
            //System.out.println("\nExecution time : " + ((double) (end - start) / (double) 1000) + "sec");
        }

    }

    // Check if string test is an attribute
    public static boolean is_an_attribute(String test, ArrayList<String> a){
        return a.contains(test);
    }

    // Check if string v is a value included in attribute's values of a
    public static boolean is_a_value_of_an_attribute(String a, String v, LinkedHashMap<String, ArrayList<String>> attributes){
        return attributes.get(a).contains(v);
    }

    // Check if string test is a data
    public static boolean is_a_data(String test, ArrayList<String> data_order_list){
        return data_order_list.contains(test);
    }

    // Check if string test is a rule
    public static boolean is_a_rule(String test){
        return rules_list.contains(test);
    }

    // Check if string test is a global rule
    public static boolean is_a_global_rule(String test){
        return global_rules_list.contains(test);
    }

    // Check if all requirements for a correct allocation are respected according definition 3.5.2
    public static boolean check_all_rules(ArrayList<GlobalRule> globalRules,
                                          LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction) {
        return  check_no_global_rules(allocationFunction) &&
                check_only_one(globalRules,allocationFunction) &&
                check_together(globalRules,allocationFunction) &&
                check_together_asterisk(globalRules,allocationFunction) &&
                check_not_together(globalRules,allocationFunction);
    }

    // Check if for all sets of requirements of type R_o : L(R_o) = 1
    // Check if for all sets of requirements of type R_c : L(R_c) = n_p
    public static boolean check_no_global_rules(LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction) {
        // Check if number of plan allocated is respected for each plans
        for (SetOfRules setR : allocationFunction.keySet()) {
            if (setR.getType().equals("o")) {
                if (!(allocationFunction.get(setR).size() == 1)) {
                    return false;
                }
            } else {
                if (!(allocationFunction.get(setR).size() == setR.getNumberOfCopy())) {
                    return false;
                }
            }
        }

        return true;
    }

    // Check if ONLY_ONE rules are respected
    // Return true if is respected, false otherwise
    public static boolean check_only_one(ArrayList<GlobalRule> globalRules,
                                         LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction) {

        HashSet<String> allocated_plans;
        for (GlobalRule g_rule : globalRules) {
            // For each rule involved in the ONLY_ONE rule check if each plan appears 1 time only
            if (g_rule.getGlobalRule().equals("ONLY_ONE")) {
                allocated_plans = new HashSet<>();
                for (SetOfRules setR : allocationFunction.keySet()) {
                    if (setR.getData().equals(g_rule.getDataType()) ||
                            setR.getData().equals(g_rule.getDataType() + "*")) {
                        for (String plan : allocationFunction.get(setR)) {
                            if (!allocated_plans.contains(plan)) {
                                allocated_plans.add(plan);
                            } else {
                                return false;
                            }
                        }
                    }
                }
            }
        }

        // check passed return true
        return true;
    }

    // Check if TOGETHER rules are respected
    // Return true if is respected, false otherwise
    public static boolean check_together(ArrayList<GlobalRule> globalRules,
                                         LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction){

        HashSet<String> plans_R2;

        for (GlobalRule g_rule : globalRules) {
            if (g_rule.getGlobalRule().equals("TOGETHER")) {
                plans_R2 = new HashSet<>();

                // add to hashset plans_R2 all plans allocated to set of requirement with data type = g_rule.getDataType2()
                for (SetOfRules setR2 : allocationFunction.keySet()) {
                    if (setR2.getData().equals(g_rule.getDataType2()) ||
                            setR2.getData().equals(g_rule.getDataType2() + "*")) {
                        plans_R2.addAll(allocationFunction.get(setR2));
                    }
                }

                // for each plan allocated to set of requirement with data type = g_rule.getDataType1(),
                // check if is contained inside plans_R2
                // if is not return false
                for (SetOfRules setR1 : allocationFunction.keySet()) {
                    if (setR1.getData().equals(g_rule.getDataType()) ||
                            setR1.getData().equals(g_rule.getDataType() + "*")) {
                        for (String plan_1 : allocationFunction.get(setR1)) {
                            if (!plans_R2.contains(plan_1)) {
                                return false;
                            }
                        }
                    }
                }
            }
        }
        return true;
    }

    // Check if TOGETHER* rules are respected
    // Return true if is respected, false otherwise
    public static boolean check_together_asterisk(ArrayList<GlobalRule> globalRules,
                                                  LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction){

        HashSet<String> plans_R2;
        boolean result;

        for (GlobalRule g_rule : globalRules) {
            if (g_rule.getGlobalRule().equals("TOGETHER*")) {
                plans_R2 = new HashSet<>();
                result = false;

                // add to hashset plans_R2 all plans allocated to set of requirement with data type = g_rule.getDataType2()
                for (SetOfRules setR2 : allocationFunction.keySet()) {
                    if (setR2.getData().equals(g_rule.getDataType2()) ||
                            setR2.getData().equals(g_rule.getDataType2() + "*")) {
                        plans_R2.addAll(allocationFunction.get(setR2));
                    }
                }

                // for each plan allocated to set of requirement with data type = g_rule.getDataType1(),
                // check if one of these is contained inside plans_R2
                // if is not return false
                for (SetOfRules setR1 : allocationFunction.keySet()) {
                    if (setR1.getData().equals(g_rule.getDataType()) ||
                            setR1.getData().equals(g_rule.getDataType() + "*")) {
                        for (String plan_1 : allocationFunction.get(setR1)) {
                            if (plans_R2.contains(plan_1)) {
                                result = true;
                                break;
                            }
                        }
                    }
                }

                if (!result)
                    return false;
            }
        }
        return true;
    }


    // Check if NOT_TOGETHER rules are respected
    // Return true if is respected, false otherwise
    public static boolean check_not_together(ArrayList<GlobalRule> globalRules,
                                             LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction){
        HashSet<String> plans_R2;

        for (GlobalRule g_rule : globalRules) {
            if (g_rule.getGlobalRule().equals("NOT_TOGETHER")) {
                plans_R2 = new HashSet<>();

                // add to hashset plans_R2 all plans allocated to set of requirement with data type = g_rule.getDataType2()
                for (SetOfRules setR2 : allocationFunction.keySet()) {
                    if (setR2.getData().equals(g_rule.getDataType2()) ||
                            setR2.getData().equals(g_rule.getDataType2() + "*")) {
                        plans_R2.addAll(allocationFunction.get(setR2));
                    }
                }

                // for each plan allocated to set of requirement with data type = g_rule.getDataType1(),
                // check if is not contained inside plans_R2
                // if is not return false
                for (SetOfRules setR1 : allocationFunction.keySet()) {
                    if (setR1.getData().equals(g_rule.getDataType()) ||
                            setR1.getData().equals(g_rule.getDataType() + "*")) {
                        for (String plan_1 : allocationFunction.get(setR1)) {
                            if (plans_R2.contains(plan_1)) {
                                return false;
                            }
                        }
                    }
                }
            }
        }
        return true;
    }

    // Search for a random allocation given ranking
    public static void random_allocation(LinkedHashMap<SetOfRules,ArrayList<String>> ranking,
                                         LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction) {
        int max_rand;
        int num_of_necessary_plans;
        ArrayList<String> temp_allocation;
        int rand_int;
        Random rand = new Random();

        // For each rule
        for (SetOfRules setR : ranking.keySet()) {
            max_rand = ranking.get(setR).size();
            temp_allocation = new ArrayList<>();
            allocationFunction.put(setR,new ArrayList<>());

            // Find quantity of plans necessary for the set of requirements
            if (setR.getType().equals("o")) {
                num_of_necessary_plans = 1;
            } else {
                num_of_necessary_plans = setR.getNumberOfCopy();
            }

            // Allocate num_of_necessary_plans random plan
            for (int i=0; i<num_of_necessary_plans; i++) {
                rand_int = rand.nextInt(max_rand);
                if(temp_allocation.contains(ranking.get(setR).get(rand_int))){
                    i=i-1;
                } else {
                    temp_allocation.add(ranking.get(setR).get(rand_int));
                }
            }

            // Set result
            allocationFunction.get(setR).addAll(temp_allocation);
        }

    }

    // Return true if temp_allocation_2 is better than temp_allocation_1 according hierarchy and ranking
    public static boolean better_allocation(LinkedHashMap<SetOfRules,ArrayList<String>> temp_allocation_1,
                                            LinkedHashMap<SetOfRules,ArrayList<String>> temp_allocation_2,
                                            LinkedHashMap<String,ArrayList<SetOfRules>> hierarchy,
                                            LinkedHashMap<SetOfRules,ArrayList<String>> ranking
                                            ) {
        // For each set of rule sum ranking of all plans
        LinkedHashMap<SetOfRules, Integer> value_1 = new LinkedHashMap<>();
        LinkedHashMap<SetOfRules, Integer> value_2 = new LinkedHashMap<>();
        for (SetOfRules setR : temp_allocation_1.keySet()) {
                for (String plan_1 : temp_allocation_1.get(setR)) {
                    value_1.putIfAbsent(setR, 0);
                    value_1.replace(setR, value_1.get(setR) + ranking.get(setR).indexOf(plan_1));
                }
                for (String plan_2 : temp_allocation_2.get(setR)) {
                    value_2.putIfAbsent(setR, 0);
                    value_2.replace(setR, value_2.get(setR) + ranking.get(setR).indexOf(plan_2));
                }
        }


        // Set variables for next step
        ArrayList<Integer> sum_value_1 = new ArrayList<>();
        ArrayList<Integer> sum_value_2 = new ArrayList<>();
        int size = 0;
        for (String data : hierarchy.keySet()) {
            if (size < hierarchy.get(data).size())
                size = hierarchy.get(data).size();
        }
        for (int i=0; i<size ;i++) {
            sum_value_1.add(0);
            sum_value_2.add(0);
        }

        // Sum of each value_1(setR) for all setR in first position of the hierarchy, and then second... until
        // max size of the hierarchy
        // the same thing was done for value_2
        int count;
        for (String data : hierarchy.keySet()) {
            count = 0;
            for (SetOfRules setR: hierarchy.get(data)) {
                sum_value_1.set(count, sum_value_1.get(count) + value_1.get(setR));
                sum_value_2.set(count, sum_value_2.get(count) + value_2.get(setR));
                count = count + 1;
            }
        }

        // Check
        // if sum_value_1.get(i) > sum_value_2.get(i) then temp_allocation_2 is better temp_allocation_1
        //          return true;
        // if sum_value_1.get(i) < sum_value_2.get(i) then temp_allocation_2 is better temp_allocation_1
        //          return false;
        // if sum_value_1.get(i) == sum_value_2.get(i) then temp_allocation_2 is equivalent to temp_allocation_1
        //          so i = i + 1;
        // if i < size then sum_value_1.get(i) == sum_value_2.get(i)
        //          so temp_allocation_1 is equivalent to temp_allocation_2 and return false
        for (int i=0; i<size;) {
            if (sum_value_1.get(i) > sum_value_2.get(i)) {
                return true;
            } else if (Objects.equals(sum_value_1.get(i), sum_value_2.get(i))) {
                i = i + 1;
            } else {
                return false;
            }
        }

        return false;
    }

    // Search the best solution considering all global rules
    public static void allocation_function_global_rules(LinkedHashMap<SetOfRules,ArrayList<String>> ranking,
                                                        LinkedHashMap<String,ArrayList<SetOfRules>> hierarchy,
                                                        ArrayList<GlobalRule> globalRules,
                                                        LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction) {
        int i = 0;
        LinkedHashMap<SetOfRules,ArrayList<String>> temp_allocation_1;
        LinkedHashMap<SetOfRules,ArrayList<String>> temp_allocation_2;

        // Search for a solution (by random allocation method)
        // if INFINITE_LOOP is true may cause an infinite loop
        do {
            temp_allocation_1 = new LinkedHashMap<>();
            random_allocation(ranking,temp_allocation_1);
            i++;
        } while (!check_all_rules(globalRules,temp_allocation_1) &&
                (i <= MAX_CYCLE || INFINITE_LOOP));

        // Search for a better solution according hierarchy (by random allocation method)
        if (!hierarchy.isEmpty()) {
            i = 0;
            do {
                temp_allocation_2 = new LinkedHashMap<>();
                random_allocation(ranking, temp_allocation_2);
                if (check_all_rules(globalRules, temp_allocation_2) &&
                        better_allocation(temp_allocation_1, temp_allocation_2, hierarchy, ranking)) {
                    temp_allocation_1.clear();
                    temp_allocation_1.putAll(temp_allocation_2);
                }
                i++;
            } while (i <= MAX_CYCLE);
        }

        // Set allocationFunction with the solution found
        allocationFunction.clear();
        allocationFunction.putAll(temp_allocation_1);
    }


    // Search the best solution considering ONLY_ONE rules
    public static void allocation_function_only_one(LinkedHashMap<SetOfRules,ArrayList<String>> ranking,
                                                    LinkedHashMap<String, ArrayList<SetOfRules>> hierarchy,
                                                    ArrayList<GlobalRule> globalRules,
                                                    LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction){
        int i = 0;
        LinkedHashMap<SetOfRules,ArrayList<String>> temp_allocation_1;
        LinkedHashMap<SetOfRules,ArrayList<String>> temp_allocation_2;

        // Search for a solution (by random allocation method)
        // Always find a solution
        do {
            temp_allocation_1 = new LinkedHashMap<>();
            random_allocation(ranking,temp_allocation_1);
            i++;
        } while (!(check_only_one(globalRules,temp_allocation_1) &&
                check_no_global_rules(temp_allocation_1)));

        // Search for a better solution according hierarchy (by random allocation method)
        if (!hierarchy.isEmpty()) {
            i = 0;
            do {
                temp_allocation_2 = new LinkedHashMap<>();
                random_allocation(ranking, temp_allocation_2);
                if ((check_only_one(globalRules, temp_allocation_2) &&
                        check_no_global_rules(temp_allocation_2)) &&
                        better_allocation(temp_allocation_1, temp_allocation_2, hierarchy, ranking)) {
                    temp_allocation_1.clear();
                    temp_allocation_1.putAll(temp_allocation_2);
                }
                i++;
            } while (i <= MAX_CYCLE);
        }

        // Set allocationFunction with the solution found
        allocationFunction.clear();
        allocationFunction.putAll(temp_allocation_1);
    }

    // It Puts in "subsets" all possible subsets of set "input"
    // In our program, given a set of requirements "input" puts in set "subsets" all possible subsets of "input".
    public static <E> void find_all_subset(List<List<E>> subsets, ArrayList<E> input,
                                       ArrayList<E> output, int index){
        // Base Condition
        if (index == input.size()) {
            subsets.add(output);
            return;
        }

        // Not Including Value which is at Index
        find_all_subset(subsets, input, new ArrayList<>(output), index + 1);

        // Including Value which is at Index
        output.add(input.get(index));
        find_all_subset(subsets, input, new ArrayList<>(output), index + 1);
    }

    // This method stops the execution if it does not exist a solution considering ONLY_ONE rules
    public static void check_if_exist_a_solution_with_only_one_rules(LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements,
                                                    LinkedHashMap<SetOfRules,ArrayList<String>> ranking,
                                                    ArrayList<GlobalRule> globalRules) {

        // Check if there is a solution for only one rules (checking all subset)
        for (GlobalRule g_rule : globalRules) {
            if (g_rule.getGlobalRule().equals("ONLY_ONE")) {
                // Find all possible subset of set of rules given the data type of global rule only one
                List<List<SetOfRules>> subsets = new ArrayList<>();
                ArrayList<SetOfRules> input = new ArrayList<>(setOfRequirements.get(g_rule.getDataType()));
                find_all_subset(subsets, input, new ArrayList<>(), 0);

                // Set of acceptable plan for the entire subset
                HashSet<String> allAcceptablePlansOfTheSubset;
                // Num of plans required for the entire subset
                int numOfPlansRequired;

                // For each subset check if the cardinality of set formed by the union of all acceptable plan for all set of
                // requirements in the subset (|P_U|) is grater or equal then the sum of plan required by each set of
                // requirements in the subset (Sum)
                // |P_U| >= Sum
                for (List<SetOfRules> subset : subsets) {
                    if (!subset.isEmpty()) {
                        allAcceptablePlansOfTheSubset = new HashSet<>();
                        numOfPlansRequired = 0;
                        for (SetOfRules setOfRules : subset) {
                            allAcceptablePlansOfTheSubset.addAll(ranking.get(setOfRules));
                            if (setOfRules.getType().equals("o")) {
                                numOfPlansRequired = numOfPlansRequired + 1;
                            } else {
                                numOfPlansRequired = numOfPlansRequired + setOfRules.getNumberOfCopy();
                            }
                        }
                        // if |P_U| < Sum then error
                        if (allAcceptablePlansOfTheSubset.size() < numOfPlansRequired) {
                            System.out.println(" (" + allAcceptablePlansOfTheSubset.size() + ") >= " + numOfPlansRequired + Main.ANSI_RESET);
                            stop("Does not exist a solution for the global rule ONLY_ONE(" + g_rule.getDataType() + ")");
                        }
                    }
                }
            }
        }
    }

    // This method stops the execution if it does not exist a solution considering ONLY_ONE rules
    // This method show shows the processing for this check
    public static void check_if_exist_a_solution_with_only_one_rules_with_print(LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements,
                                                                                LinkedHashMap<SetOfRules,ArrayList<String>> ranking,
                                                                                ArrayList<GlobalRule> globalRules) {

        // Check if there is a solution for only one rules (checking all subset)
        System.out.println("\nCheck if exists a solution considering ONLY_ONE Rules");
        System.out.println(ANSI_WHITE + "for all subset of SetOfRule R(d) with \"d\" included in an ONLY_ONE rule" + ANSI_RESET);
        System.out.println(ANSI_WHITE + "(cardinality of set of all acceptable plans for the subset >= total of plans required by the subset)" + ANSI_RESET);
        for (GlobalRule g_rule : globalRules) {
            if (g_rule.getGlobalRule().equals("ONLY_ONE")) {
                // Find all possible subset of set of requirements given the data type of global rule only one
                List<List<SetOfRules>> subsets = new ArrayList<>();
                ArrayList<SetOfRules> input = new ArrayList<>(setOfRequirements.get(g_rule.getDataType()));
                Main.find_all_subset(subsets, input, new ArrayList<>(), 0);

                // Set of acceptable plan for the entire subset
                HashSet<String> allAcceptablePlansOfTheSubset;
                // Num of plans required for the entire subset
                int numOfPlansRequired;

                // For each subset check if the cardinality of set formed by the union of all acceptable plan for all set of
                // requirements in the subset (|P_U|) is grater or equal then the sum of plan required by each set of
                // requirements in the subset (Sum)
                // |P_U| >= Sum
                System.out.println("\t" + "ONLY_ONE(" + g_rule.getDataType() + ") :");
                for (List<SetOfRules> subset : subsets) {
                    if (!subset.isEmpty()) {
                        allAcceptablePlansOfTheSubset = new HashSet<>();
                        numOfPlansRequired = 0;
                        System.out.print("\t\t");
                        for (SetOfRules setOfRules : subset) {
                            System.out.print(setOfRules.printRuleName() + " ");
                            allAcceptablePlansOfTheSubset.addAll(ranking.get(setOfRules));
                            if (setOfRules.getType().equals("o")) {
                                numOfPlansRequired = numOfPlansRequired + 1;
                            } else {
                                numOfPlansRequired = numOfPlansRequired + setOfRules.getNumberOfCopy();
                            }
                        }
                        if (allAcceptablePlansOfTheSubset.size() >= numOfPlansRequired) {
                            System.out.print(Main.ANSI_GREEN + "|{");
                            Iterator<String> itr = allAcceptablePlansOfTheSubset.iterator();
                            int k = 0;
                            while (itr.hasNext()) {
                                if (k < allAcceptablePlansOfTheSubset.size() - 1)
                                    System.out.print(itr.next() + ",");
                                else
                                    System.out.print(itr.next() + "}|");
                                k++;
                            }
                            System.out.println(" (" + allAcceptablePlansOfTheSubset.size() + ") >= " + numOfPlansRequired + Main.ANSI_RESET);
                        // if |P_U| < Sum then error
                        } else {
                            System.out.print(Main.ANSI_RED + "|{");
                            Iterator<String> itr = allAcceptablePlansOfTheSubset.iterator();
                            int k = 0;
                            while (itr.hasNext()) {
                                if (k < allAcceptablePlansOfTheSubset.size() - 1)
                                    System.out.print(itr.next() + ",");
                                else
                                    System.out.print(itr.next() + "}|");
                                k++;
                            }
                            System.out.println(" (" + allAcceptablePlansOfTheSubset.size() + ") >= " + numOfPlansRequired + Main.ANSI_RESET);
                            stop("Does not exist a solution for the global rule ONLY_ONE(" + g_rule.getDataType() + ")");
                        }
                    }
                }
            }
        }
    }

    // Set the allocation function without considering global requirements
    public static void allocation_function_no_global_rules(LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements,
                                                           LinkedHashMap<SetOfRules,ArrayList<String>> ranking,
                                                           LinkedHashMap<SetOfRules,ArrayList<String>> allocationFunction){
        // Check if there are enough plans for each set of requirements
        for (String attr : setOfRequirements.keySet()) {
            for (SetOfRules setR : setOfRequirements.get(attr)) {
                if (setR.getType().equals("o") && ranking.get(setR).size() < 1){
                    stop("There are not enough plans to be allocated for the set of requirements " + setR.printRuleName() +
                            "\n\tplans available : " + ranking.get(setR).size() + "\n\tplans needed : 1");
                } else if (setR.getType().equals("c") && ranking.get(setR).size() < setR.getNumberOfCopy()){
                    stop("There are not enough plans to be allocated for the set of requirements " + setR.printRuleName() +
                            "\n\tplans available : " + ranking.get(setR).size() + "\n\tplans needed : " + setR.getNumberOfCopy());
                }
            }
        }

        // Assign plans based on ranking
        for (String attr : setOfRequirements.keySet()){
            for(SetOfRules setR : setOfRequirements.get(attr)){
                allocationFunction.put(setR, new ArrayList<>());
                if (setR.getType().equals("o")){
                    allocationFunction.get(setR).add(ranking.get(setR).get(0));
                } else {
                    for (int i=0; i<setR.getNumberOfCopy(); i++) {
                        allocationFunction.get(setR).add(ranking.get(setR).get(i));
                    }
                }
            }
        }
    }

    // Calculate ranking of plans based on distance
    public static void calculate_ranking(LinkedHashMap<SetOfRules,LinkedHashMap<String,ScoreVector>> scoreVectors,
                                         LinkedHashMap<SetOfRules, ArrayList<String>> ranking){
        for (SetOfRules setR : scoreVectors.keySet()){
            ranking.put(setR, new ArrayList<>());
            ranking.get(setR).addAll(scoreVectors.get(setR).keySet());
            ranking.get(setR).remove("p_top");
            // Sort ArrayList in ascending order based on plan distance from ideal plan
            ranking.get(setR).sort((p1, p2) -> {
                double delta = scoreVectors.get(setR).get(p1).getDistance() - scoreVectors.get(setR).get(p2).getDistance();
                if (delta > 0) {
                    return 1;
                } else if (delta < 0) {
                    return -1;
                } else {
                    return 0;
                }
            });
        }
    }

    // Calculate the score vector or weighed score vector for each plan and for the p_top plan
    public static void score_vector_calculation(String dominantRelationship,
                                                ArrayList<String> attributeOrderList,
                                                LinkedHashMap<String,ArrayList<String>> plans,
                                                LinkedHashMap<SetOfRules,ArrayList<String>> setOfAcceptablePlans,
                                                LinkedHashMap<SetOfRules,LinkedHashMap<String,Preference>> preferences,
                                                LinkedHashMap<SetOfRules,LinkedHashMap<String,ScoreVector>> scoreVectors){
        ScoreVector temp_vector;
        String temp_value;

        // For each SetOfRules
        for (SetOfRules setR : preferences.keySet()) {
            scoreVectors.put(setR,new LinkedHashMap<>());

            // Calculate the score vector or weighed score vector for Ideal plan
            temp_vector = new ScoreVector();
            for (String attr : preferences.get(setR).keySet()) {
                temp_vector.add_value(preferences.get(setR).get(attr).getTopValue());
                temp_vector.add_attribute(attr);
                if(dominantRelationship.equals("D")) {
                    temp_vector.add_score(preferences.get(setR).get(attr).getTopScore());
                } else if(dominantRelationship.equals("DP")) {
                    temp_vector.add_score(preferences.get(setR).get(attr).getTopScore() * (double) preferences.get(setR).get(attr).getWeight());
                }
            }
            scoreVectors.get(setR).put("p_top",temp_vector);

            //  Calculate the score vector or weighed score vector for acceptable plans
            for (String plan : setOfAcceptablePlans.get(setR)) {
                temp_vector = new ScoreVector();
                for (String attr : preferences.get(setR).keySet()) {
                    temp_value = plans.get(plan).get(attributeOrderList.indexOf(attr));
                    temp_vector.add_value(temp_value);
                    temp_vector.add_attribute(attr);
                    if(dominantRelationship.equals("D")) {
                        temp_vector.add_score(preferences.get(setR).get(attr).getScoreFromValue(temp_value));
                    } else if (dominantRelationship.equals("DP")) {
                        temp_vector.add_score(preferences.get(setR).get(attr).getScoreFromValue(temp_value) *
                                (double) preferences.get(setR).get(attr).getWeight());
                    }
                }
                scoreVectors.get(setR).put(plan,temp_vector);
            }
        }

        // Calculate the distance for each acceptable plane from the ideal plane
        ScoreVector ideal_plan;
        for (SetOfRules setR : scoreVectors.keySet()){
            ideal_plan = scoreVectors.get(setR).get("p_top");
            for (String plan : scoreVectors.get(setR).keySet()) {
                if (!plan.equals("p_top")) {
                    scoreVectors.get(setR).get(plan).calculate_distance(ideal_plan);
                }
            }
        }
    }

    // Define set of preference relations defined in definition 3.4.2 pag 43
    public static void preference_relation(LinkedHashMap<SetOfRules,LinkedHashMap<String,Preference>> preferences,
                                           LinkedHashMap<SetOfRules,LinkedHashMap<String,ArrayList<String>>> restrictedDomain,
                                           LinkedHashMap<String,ArrayList<String>> attributes,
                                           File[] file_list_of_preference,
                                           File[] file_list_of_weight_function) throws IOException {
        // Default definition of set of preference relations and weight function
        // Default order is attribute by attribute as defined in the LinkedHashMap attributes.
        for (SetOfRules setR : restrictedDomain.keySet()) {
            preferences.put(setR, new LinkedHashMap<>());
            for (String attr : restrictedDomain.get(setR).keySet()) {
                if (restrictedDomain.get(setR).get(attr).size() > 1) {
                    preferences.get(setR).put(attr, new Preference());
                    for (String val : attributes.get(attr)){
                        if (restrictedDomain.get(setR).get(attr).contains(val)) {
                            preferences.get(setR).get(attr).addSetOfPreference( new ArrayList<>() {{ add(val);}} );
                        }
                    }
                }
            }
        }

        // User definition of set of preference relations
        ReadPrintParse.read_preferences(preferences, restrictedDomain, file_list_of_preference);

        // Definition of score function
        score_function(preferences);
        // User definition of weight function, if is not defined default weight score is 1.
        // Define weight function according definition 3.4.4. pag.47
        ReadPrintParse.weight_function(preferences, file_list_of_weight_function);
    }

    // Calculate the score function according definition 3.4.3 pag.46
    public static void score_function(LinkedHashMap<SetOfRules,LinkedHashMap<String,Preference>> preferences) {
        double score;
        int k;
        // For all attr calculate score function
        for (SetOfRules setR : preferences.keySet()){
            for (String attr : preferences.get(setR).keySet()) {
                k = preferences.get(setR).get(attr).getPreferenceRelation().size();
                for(int i=0; i<k; i++){
                    // score = 1 - (k / i)
                    score = (double) 1 - ((double) i / (double) k);
                    preferences.get(setR).get(attr).addScore(score);
                }
            }
        }
    }

    // Identifies restricted domains according to definition 3.4.1 at pag.42
    public static void restricted_domain(LinkedHashMap<SetOfRules,LinkedHashMap<String,ArrayList<String>>> restrictedDomain,
                                         LinkedHashMap<String,ArrayList<String>> attributes,
                                         LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements) {
        // Cycle all SetOfRules
        for(String s : setOfRequirements.keySet()) {
            for (SetOfRules setR : setOfRequirements.get(s)) {

                restrictedDomain.put(setR, new LinkedHashMap<>());
                // Check if an attribute appears in a term and add the respective attribute values
                for (Term t : setR.getTerms()) {
                    restrictedDomain.get(setR).put(t.getAttribute(), new ArrayList<>());
                    if (t.getIncludeValues()) {
                        restrictedDomain.get(setR).get(t.getAttribute()).addAll(t.getValues());
                    } else {
                        restrictedDomain.get(setR).get(t.getAttribute()).addAll(attributes.get(t.getAttribute()));
                        restrictedDomain.get(setR).get(t.getAttribute()).removeAll(t.getValues());
                    }
                }

                // Check if an attribute appears in an "ALL" rule and add the respective attribute values
                for (Rule r : setR.getRules()) {
                    if(r.getRuleName().equals("ALL")) {
                        for(Term t : r.getTerms()){
                            restrictedDomain.get(setR).put(t.getAttribute(), new ArrayList<>());
                            if (t.getIncludeValues()) {
                                restrictedDomain.get(setR).get(t.getAttribute()).addAll(t.getValues());
                            } else {
                                restrictedDomain.get(setR).get(t.getAttribute()).addAll(attributes.get(t.getAttribute()));
                                restrictedDomain.get(setR).get(t.getAttribute()).removeAll(t.getValues());
                            }
                        }
                    }
                }

                // Add values for attributes not present in terms and "ALL" rules
                for (String attr : attributes.keySet()) {
                    if (!restrictedDomain.get(setR).containsKey(attr)) {
                        restrictedDomain.get(setR).put(attr, new ArrayList<>());
                        restrictedDomain.get(setR).get(attr).addAll(attributes.get(attr));
                    }
                }
            }
        }
    }

    // Check if there are enough acceptable plans for each set of requirements
    // >= 1 for R_o
    // >= n_p for R_c1
    public static void check_if_there_enough_acceptable_plans(LinkedHashMap<SetOfRules,ArrayList<String>> setOfAcceptablePlans,
                                                              LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements) {
        for (String data : setOfRequirements.keySet()) {
            for (SetOfRules setR : setOfRequirements.get(data)) {
                if (setR.getType().equals("o")) {
                    if (!(setOfAcceptablePlans.get(setR).size() >= 1)) {
                        stop("The number of acceptable plans for " + setR.printRuleName() + " is insufficient!");
                    }
                } else {
                    if (!(setOfAcceptablePlans.get(setR).size() >= setR.getNumberOfCopy())) {
                        stop("The number of acceptable plans for " + setR.printRuleName() + " is insufficient!");
                    }
                }
            }
        }
    }

    // Add in LinkedHashMap setOfAcceptablePlans for each SetOfRules the plan p if it satisfies SetOfRule's rule and term
    // Identifies acceptable plans according to definition 3.3.3 at pag.40
    public static void acceptable_plans(LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements,
                                            LinkedHashMap<String,ArrayList<String>> plans,
                                            ArrayList<String> attribute_order_list,
                                            LinkedHashMap<SetOfRules, ArrayList<String>> setOfAcceptablePlans) {
        // Contains indexes of attributes that must be checked
        ArrayList<ArrayList<Integer>> attribute_index_for_verify;
        // Values of attributes that must be checked
        ArrayList<ArrayList<String>> value_of_plan;
        ArrayList<String> temp_string;
        ArrayList<Integer> temp_int;

        // Cycle all SetOfRules
        for(String s : setOfRequirements.keySet()) {
            for (SetOfRules setR : setOfRequirements.get(s)){

                // Create the set of acceptable plans for the set of rules setR
                setOfAcceptablePlans.put(setR, new ArrayList<>());

                // Search attribute indexes which will be put in attribute_index_for_verify
                attribute_index_for_verify = new ArrayList<>();
                for(ArrayList<String> list_av : setR.arrayList_list_for_verify()){
                    temp_int = new ArrayList<>();
                    for (String av : list_av){
                        temp_int.add(attribute_order_list.indexOf(av));
                    }
                    attribute_index_for_verify.add(temp_int);
                }

                for (String plan : plans.keySet()){
                    // Search respective values for each plan
                    value_of_plan = new ArrayList<>();
                    for (ArrayList<Integer> list_avi : attribute_index_for_verify){
                        temp_string = new ArrayList<>();
                        for (int avi : list_avi){
                            temp_string.add(plans.get(plan).get(avi));
                        }
                        value_of_plan.add(temp_string);
                    }

                    // If the plan satisfies the SetOfRule, it is added to the set of acceptable plans
                    if(setR.verify(value_of_plan)){
                        setOfAcceptablePlans.get(setR).add(plan);
                    }
                }
            }
        }
    }

    // Definition 3.2.1 pag.33
    // In each requirement's set there is at most 1 occurrence of an attribute for all terms and rules "ALL".
    // Each rule must contain at most 1 occurrence of an attribute.
    public static void check_if_requirements_are_well_defined(LinkedHashMap<String,ArrayList<SetOfRules>> setOfRequirements){
        ArrayList<String> a_in_term_all;
        ArrayList<String> a_in_rule;

        // Cycle all SetOfRules
        for(String s : setOfRequirements.keySet()) {
            for (SetOfRules setR : setOfRequirements.get(s)){
                a_in_term_all = new ArrayList<>();

                // Verify that an attribute only appears 1 time in an ALL term or rule
                for (Term t : setR.getTerms()){
                    if (!a_in_term_all.contains(t.getAttribute()))
                        a_in_term_all.add(t.getAttribute());
                    else
                        stop("Set of requirements " + setR.printRuleName() + " is malformed."
                                + "\nThere is more than 1 term that contains the attribute \"" +
                                t.getAttribute() + "\"!");
                }
                for (Rule r : setR.getRules()){
                    if(Objects.equals(r.getRuleName(), "ALL")) {
                        for (Term t : r.getTerms()) {
                            if (!a_in_term_all.contains(t.getAttribute()))
                                a_in_term_all.add(t.getAttribute());
                            else
                                stop("Set of requirements " + setR.printRuleName() + " is malformed."
                                        + "\nThere is more than 1 term or rule ALL that contains the attribute \"" +
                                        t.getAttribute() + "\" or \n" +
                                        "there is more than 1 occurrence of the the attribute \"" +
                                        t.getAttribute() + "\" " + "in an ALL rule!");
                        }
                    }
                }

                // Verify that each rule contains only one occurrence of an attribute
                for (Rule r : setR.getRules()){
                    a_in_rule = new ArrayList<>();
                    for (Term t : r.getTerms()) {
                        if (!a_in_rule.contains(t.getAttribute()))
                            a_in_rule.add(t.getAttribute());
                        else
                            stop("Set of requirements " + setR.printRuleName() + " is malformed." +
                                    "\nThe Rule \"" + r + "\" contains more than 1 occurrence an attribute \"" +
                                    t.getAttribute() + "\"!");
                        if (Objects.equals(r.getRuleName(), "IF-THEN")) {
                            for (Term t2 : r.getTerms2()) {
                                if (!a_in_rule.contains(t2.getAttribute()))
                                    a_in_rule.add(t2.getAttribute());
                                else
                                    stop("Set of requirements " + setR.printRuleName() + " is malformed." +
                                            "\nThe Rule \"" + r + "\" contains more than 1 occurrence of attribute \"" +
                                            t2.getAttribute() + "\"!");
                            }
                        }
                    }
                }
            }
        }
        System.out.println(ANSI_GREEN + "Set of Requirements (R) is well-defined according to definition 3.2.1" + ANSI_RESET);
    }

    // Pause execution
    static String pause(String m){
        Scanner input = new Scanner(System.in);
        System.out.print(ANSI_YELLOW + m + ANSI_RESET);
        return input.next();
    }

    // Stop execution
    static void stop(String m){
        System.out.println();
        System.out.println(ANSI_RED + m + ANSI_RESET);
        System.exit(0);
    }
}